...
for group_df in temp_list:
    for column in group_df.columns:
        ...
        group_df[column] = value.mask(outliers).fillna(method='ffill')
...
for data in new_temp_list:
    ...
    with torch.no_grad():
        ...
        preds_inverse = pred * (max_val[0] - min_val[0]) + min_val[0]
        print(preds_inverse.astype(int))
        # predicted_cases = np.expand_dims(preds, axis=0).flatten()
        res.append(preds_inverse.astype(float))

combined_array = np.concatenate(res)


submit_data['pred'] =combined_array
